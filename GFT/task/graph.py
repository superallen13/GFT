...
from utils.eval import evaluate_graph, task2metric
...
def ft_graph(model, dataset, loader, split, labels, num_classes, optimizer, params, scheduler=None, **kwargs):
    model.train()
...
def eval_graph(model, dataset, loader, split, labels, num_classes, params, **kwargs):
    ...
    if setting == 'standard':
        ...
        train_value = eval_graph_single(model=model, dataset=dataset, loader=train_loader, split=split, labels=labels,
                                        num_classes=num_classes, params=params, setting=setting)
...
def eval_graph_single(model, dataset, loader, split, labels, num_classes, params, **kwargs):
    model.eval()
    device = get_device_from_model(model)
    setting = kwargs["setting"]
    ...
    y[y == 0] = -1
    pred = torch.cat(pred_list, dim=0).detach().cpu().numpy()
    roc = evaluate_graph(pred, y)

    return roc


# This works for few-shot, zero-shot, and in-context
def eval_graph_few_shot(model, dataset, loader, split, labels, num_classes, params, **kwargs):
    model.eval()
    device = get_device_from_model(model)
    setting = kwargs["setting"]

    assert setting in ["few_shot", "zero_shot", "in_context"]
    ...
    for i in range(n_task):
        ...
        train_values.append(roc)
        val_values.append(roc)
        if valid_as_test:
            test_values.append(roc)
